...
def generate(image, container, config, **kwargs):
    ...
    if default_template_name:
        default_wrapper = WrapperScript(**constructor_kwargs)
        # include_container_dir not set -> only look in the global locations
        default_wrapper.load_template(default_template_name)
    ...
    for alias in aliases:
        ...
        if custom_wrapper_option_name in alias:
            wrapper = WrapperScript(**constructor_kwargs)
            wrapper.load_template(
                alias[custom_wrapper_option_name], include_container_dir=True
            )
    ...
    for alias, template_name in listing.items():
        ...
        wrapper.load_template(template_name, include_container_dir=True)
